...
function buildRewritePrompt(roleTitle: string, beforeText: string, userFactsText: string): string {
  const factsSection = userFactsText?.trim() ? userFactsText.trim() : '(brak dodatkowych faktów)';

  return [
    `Jesteś profesjonalnym rekruterem i ekspertem od pisania CV (Resume Writer).`,
    `Twoim zadaniem jest ulepszenie sekcji "Doświadczenie" użytkownika, tworząc Impact Bullet Points.`,
    ``,
    `KONTEKST:`,
    `Rola: ${roleTitle}`,
    `Oryginalny opis (BEFORE):`,
    `"${beforeText}"`,
    ``,
    `NOWE DANE OD UŻYTKOWNIKA (Fakty, które udało się ustalić w rozmowie):`,
    `${factsSection}`,
    ``,
    `INSTRUKCJE:`,
    `1. Przeanalizuj "Oryginalny opis" i "Nowe dane".`,
    `2. Połącz je w spójne punkty. NIE wypisuj nowych danych jako osobnych punktów (chyba że to konieczne). Wpleć je w kontekst.`,
    `   ZŁE: "- Obsługa klienta. - Skala: 50 zgłoszeń."`,
    `   DOBRE: "- Obsługa 50 zgłoszeń klienckich dziennie, dbając o terminowe rozwiązywanie problemów."`,
    `3. Popraw styl: Zamień bierne sformułowania (np. "Odpowiedzialny za sprzedaż") na aktywne czasowniki (np. "Realizowałem cele sprzedażowe", "Pozyskiwałem").`,
    `4. Nie zmyślaj liczb, których nie ma w "Nowych danych" ani w "BEFORE".`,
    ``,
    `WYMAGANY FORMAT WYJŚCIA:`,
    `=== BEFORE (${roleTitle}) ===`,
    `(Tutaj wklej oryginalny tekst usera bez zmian)`,
    `=== AFTER (${roleTitle}) ===`,
    `Wersja A (Profesjonalna):`,
    `- [Styl korporacyjny: Aktywny Czasownik + Zadanie + Kontekst]`,
    `- [Uporządkuj chaotyczne opisy w 3-5 konkretnych punktów]`,
    `Wersja B (Nastawiona na Wynik):`,
    `- [Styl Impact: Wyeksponuj liczby i wyniki z sekcji "Nowe dane"]`,
    `- [Jeśli user podał metryki, muszą się tu znaleźć]`,
    `- [Mocne słownictwo: Zoptymalizowałem, Zwiększyłem, Wdrożyłem]`,
    ``,
